...
| Phase 1 — 포스트 최신화 | **우선순위 목록 완료** (커밋 20~22, 33~40) |
...
### Phase 1 우선순위 문서 — 전부 처리됨

| 문서 | 처리 | 커밋 |
|---|---|---|
| `dev/JVM/version.pug` | Java 26까지 갱신 | 22 |
| `dev/JVM/java_ee.pug` | Jakarta EE 11 기준, 구 본문 `+legacy` | 33 |
| `dev/python/basic.pug` | 3.14 기준, 3.9~3.14 절 추가 | 34 |
| `dev/JVM/jpa.pug` | Jakarta Persistence 3.2 | 36 |
| `dev/dotnet/csharp.pug`, `wpf.pug` | .NET 10 / C# 14 | 37 |
| `dev/JVM/spring_framework.pug` | Spring 7 / Boot 4 | 39 |
| `dev/JVM/spring_servlet.pug` | 위와 동일 + JSP 위치 | 39 |
| `dev/python/standard.pug` | 3.9~3.14 stdlib 변화 | 40 |
| `dev/rpi.pug` | Pi 5 / Trixie / picamera2 / OpenCV 4 | 40 |
| ~~`project/error.pug`~~ | **대상 아님** — 날짜가 붙은 트러블 로그 | — |

### 남은 것

| 항목 | 성격 |
|---|---|
| 접어둔 구 본문의 미이스케이프 `>` | 기존 문제. `java_ee` 10건, `jpa` 13건, `spring_framework` 24건, `spring_servlet` 21건, `standard` 5건. HTML 파서가 관대해 렌더는 정상이나 일관성이 없다 |
| `posts.json` 미등록 pug 11건 (경고 W1) | 공개 여부 결정 필요 |
| `dateModified` 를 git 커밋 날짜 기반으로 | 머신 간 재현성 문제 |
| Repositories 빌드 검증 | 문서 맨 아래 검증 체크리스트 |
| `posts.json` 과 pug 제목 이중 관리 | 39건 불일치. 대부분 의도적이나 스키마 정리 여지 있음 |
...
| 38 | Spring 패치 버전 정정 | fix | 커밋 30 오류 | 7.0.0 → 7.0.8, 4.0.0 → 4.0.7 |
| 39 | Spring Framework·Servlet 문서 갱신 | feat | Phase 1-2 (1·2순위) | **불일치 배너 4개 전부 제거** |
| 40 | Python 표준 라이브러리·Raspberry Pi 문서 갱신 | feat | Phase 1-2 (3순위) | Phase 1 우선순위 목록 완료 |
...
## 41. 접힌 구 본문에 남아 있던 배너 제거, 진행 상태 정리

**변경 파일**: `pugs/dev/JVM/java_ee.pug`, `posts/dev/JVM/java_ee.html`, `docs/modernization-log.md`

커밋 33에서 `java_ee.pug` 를 접을 때 **임시 배너를 제거하는 정규식이 매칭되지 않아 배너가 구 본문과 함께 접혔다.** 접힌 블록 안에서 "본문 최신화는 진행 중입니다" 라고 말하는 상태였다.

전수 검사(`posts/**` 617개)로 발견해 제거했다. **불일치 배너 잔존 0건.**
...
|---|---|
| Phase 0 — 결함 수정 | 완료 |
| Phase 1 — 포스트 최신화 | 우선순위 목록 완료 (9개 문서) |
| Phase 2 — Repositories | 재작성 완료, **빌드 미검증** |
| Phase 3 — SEO·CI·문서화 | 완료 |

## 이 작업에서 실제로 고친 결함

계획에 없었지만 작업 중 발견해 고친 것들이다.

| 결함 | 발견 경위 |
|---|---|
| **사이트맵 URL 251개가 전부 무효** | 코드 읽다 발견. `robots.txt` 가 가리키는 파일이라 색인에 직접 영향 |
| **UA 태그가 3년째 아무것도 수집하지 않음** | 〃 |
| **CI가 첫 실행에서 반드시 실패할 상태** | 독립 리뷰 |
| **`npm run build-all` 은 존재하지 않는 스크립트** | Node 정리 중 |
| **코드 버튼이 디렉터리를 가리켜 동작 안 함** | 정합성 검사 E6 보완 후 검출 |
| **`@CleanUp` 절이 `@NonNull` 예제를 열고 있었음** | 위 수정 중 육안 확인 |
| **Gradle 7·8에서 제거된 API 3종** | 툴체인 상향 중 |
| **Spring 패치 버전이 8개월 뒤처짐** | 공식 데이터 대조 |
| 깨진 코드 참조 2건 (제거된 서브모듈) | 초기 인벤토리 |
| 고아 HTML 355개 | 〃 |
...
| 오류 | 어떻게 잡았나 |
|---|---|
| "타입 검사 통과" 서술이 틀림 (`source/build.ts` 만 검사) | 독립 리뷰 |
| UA 태그 제거가 38건 미완 | 〃 |
| 홈페이지가 `TechArticle` 로 나감 | 〃 |
| `category` 다중 소속을 쉼표로 설명 (실제는 배열) | 〃 |
| 계획서에 "`local.properties` 가 커밋됨" 이라 적음 (사실 아님) | `git ls-files` 확인 |
| `jakarta.servlet:javax.servlet-api` 라는 없는 좌표 생성 | 적용 후 grep |
| `7.0.0RELEASE` 라는 버전 문자열 생성 | 〃 |
| 미참조라는 이유로 사용 중인 도구를 삭제 대상에 포함 | 사용자 지적 |
| "JDK 설치 불가" 라고 확인 없이 단정 | 사용자 질문 |
| "브라우저 권한 문제" 라고 오진 | 〃 |
| 접힌 본문에 배너 잔존 | 전수 검사 |

---

## 42. `dateModified` 를 git 이력 기반으로 전환 — mtime 의존 제거

**변경 파일**: `tools/build-doc-dates.mjs`·`tools/refactor-commits.json`·`source/doc-dates.json` (신규), `source/build.ts`, `tools/check-integrity.mjs`, `package.json`, `CLAUDE.md`, `files/posts-compressed.json`, `posts/**` 262개

`CLAUDE.md` 에 경고로만 남겨두었던 문제를 실제로 해결했다.

> ~~`dateModified` 는 pug 파일의 mtime을 쓴다. 새로 클론한 저장소는 모든 파일의 mtime이 체크아웃 시각이므로, 다른 머신에서 빌드를 한 번만 돌려도 263개 페이지의 날짜가 그날로 덮이고 대량 diff가 생긴다.~~

### 왜 `git log -1` 로는 안 되나

이 저장소에는 **pug 를 100개 이상 한꺼번에 건드린 커밋이 여럿 있다.** 단순히 마지막 커밋 날짜를 쓰면 문서 대부분의 갱신일이 리팩터링 날짜로 뭉개진다.

| 커밋 | 날짜 | 파일 | 성격 |
|---|---|---:|---|
| `9c3487ee` | 2024-02-08 | 201 | `+post` 옵션 뒤 후행 쉼표 추가 |
| `c30a1e8c` | 2024-12-03 | 150 | 표 마크업을 `+table()` mixin 으로 |
| `b7d2d25f` | 2024-12-02 | 150 | `table.no-sort` → w3 클래스 정리 |
| `4cb3b657` | 2024-12-05 | 135 | 디렉터리 이동에 따른 include 경로 조정 |
| `dfb37eb5` | 2025-03-24 | 129 | 도서 메타를 `+bookInfo()` mixin 으로 |

### 두 단계로 거른다

**1. 선언된 리팩터링 커밋 제외** — `tools/refactor-commits.json` 에 SHA 와 사유를 적어둔다. 문자는 바뀌었지만 의미가 같은 경우(mixin 교체, 경로 조정, 후행 쉼표)를 사람이 판단해 등록한다.

**2. 파일 단위 공백 판정** — 나머지 커밋에서도 그 파일의 변경이 공백뿐이면 제외한다. 커밋 단위 제외만으로는 부족하다. **일부 파일만 실제 수정된 혼합 커밋**에서 들여쓰기만 바뀐 파일을 걸러내야 하기 때문이다. `+legacy` 로 본문을 접는 작업이 정확히 이 경우다.

### 작업 중 잡은 오분류 2건

**`2eb96b5d`(2021-08-01, 106개)를 처음에 "디렉터리 이동"으로 분류했는데 틀렸다.** `--name-status` 로 확인하니 **105개가 `A`(신규 추가)** 였다. 문서 105개를 한꺼번에 올린 실제 내용 커밋이다. 제외 목록에서 빼고, 같은 착오가 반복되지 않도록 JSON 에 `$notExcluded` 항목으로 사유를 남겼다.

**rename 때문에 이력 사슬이 끊기는 문제.** 처음 실행했을 때 262개 중 **91개**가 "실질 변경 이력 없음" 으로 떨어졌다. `book/0/001.pug` 를 추적해보니 `4cb3b657` 에서 경로가 바뀌었고, 그 이전 이력이 옛 경로에 달려 있어 보이지 않았다. 패치의 `rename from` 을 읽어 옛 경로를 현재 경로로 접는 처리를 넣었더니 **91개 → 2개**로 줄었다.

| | 처음 | rename 추적 후 |
|---|---:|---:|
| 실질 변경으로 판정된 파일 변경 | 833 | 939 |
| 이력 없어 대체된 문서 | 91 | **2** |
...
```
전:  2021:2   2024:36  2025:169  2026:55     ← 2025 에 몰림 (리팩터링 날짜)
후:  2021:18  2022:6   2023:35  2024:50  2025:100  2026:53
```

홈의 "최근 갱신" 목록도 실제로 고친 문서를 가리킨다.

```
2026-08-01  Jakarta EE
2026-08-01  Spring Framework
2026-08-01  Web Servlet
2026-08-01  표준 라이브러리
2026-08-01  Raspberry Pi 3 + OpenCV(python)
```

### 빌드 시점에는 git 이 필요 없다

계산 결과를 `source/doc-dates.json` 에 커밋해둔다. 빌드는 이 파일만 읽으므로 **CI, tarball, 어느 머신에서든 같은 날짜가 나온다.** git 이 없는 환경에서도 동작한다.

`build.ts` 는 워커 init 메시지로 이 맵을 전달하고, 워커는 `docModified()` 로 조회한다. 맵에 없는 문서만 mtime 으로 대체한다.

홈의 "최근 갱신" 목록(`posts.json` 의 `mtimeMs`)도 같은 값을 쓴다.

### 검증

**핵심 검증 — mtime 을 전부 바꿔도 산출물이 동일한가**

```bash
$ find pugs -name "*.pug" -exec touch {} +    # 새 클론 상황 재현
$ (재렌더)
✅ mtime 을 전부 바꿨는데도 263개 산출물이 동일하다
```

그 외

- 타입 검사 오류 0건
- 정합성 검사 오류 0건 (W4 경고 0건 — 262개 전부 날짜 보유)
- 리다이렉트 스텁 308건 무결

### 운영

- 문서를 추가·수정해 **커밋한 뒤** `npm run dates` 를 돌린다. 빼먹으면 정합성 검사 **W4** 가 알려준다
- 새 리팩터링 커밋을 만들었다면 `tools/refactor-commits.json` 에 SHA 를 추가하고 다시 돌린다
