    ...
    public static void main(String[] args) {
        ...
        while (testCase-- > 0) {
            ...
            for (int i = 1; i <= cycle; ++i) {
                ...
                sum[i][1] = max(sum[i - 1][0], sum[i - 1][1] + money[3 * i - 3], sum[i - 1][2] + money[3 * i - 3])
                        + money[3 * i - 1];
                sum[i][2] = max(sum[i - 1][0] + money[3 * i - 2],
                        sum[i - 1][1] + max(money[3 * i - 3], money[3 * i - 2]),
                        sum[i - 1][2] + money[3 * i - 3] + money[3 * i - 2]);
            }
