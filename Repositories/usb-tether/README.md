...
> **먼저 프록시 예외를 설정하세요.** 클라이언트가 프록시를 쓰고 있으면 파일 서버 접속이 **HTTP 403** 으로 막힙니다. 프록시가 사설 주소로 가지 않도록 만들어 두었기 때문입니다(`DestinationFilter`). 파일 서버는 하필 그 사설 주소에 있으므로, **직접 연결**해야 합니다.
>
> - **Windows 시스템 프록시**를 쓰는 경우: 설정 → 네트워크 및 인터넷 → 프록시 → 수동 프록시 설정의 예외 목록에 `192.168.49.*` 추가
> - **Edge 확장**(`Chrome Proxy Extension`)을 쓰는 경우: 기본 예외 목록에 `192.168.49.0/24` 가 이미 들어 있으므로 그대로 두면 됩니다
>
> 프록시를 통해 우회하게 만들 수는 없습니다. `DestinationFilter` 에 예외를 두어도 `PeerFilter` 가 또 막습니다 — 프록시가 대신 연결하면 상대 주소가 `192.168.49.1` 이 되어 "폰 내부 앱"으로 판정됩니다. 두 겹을 다 뚫으면 폰 내부 앱 차단이 무너집니다.
