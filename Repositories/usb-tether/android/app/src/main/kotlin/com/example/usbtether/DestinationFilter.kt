...
object DestinationFilter {
    ...
    internal fun isBlocked(address: InetAddress): Boolean {
        ...
        if (address is Inet6Address) {
            ...
            // 프리픽스째로 막는다.
            //
            // 과차단 가능성: `::/96` 과 `2002::/16` 은 폐기된 형태라 대가가 없다.
            // `64:ff9b::/32` 는 IPv6-only 망에서 DNS64 가 실제로 돌려주는 값이므로
            // 정상 용도가 있다. 다만 이 앱의 클라이언트는 IPv4 P2P 대역에 있고,
            // 스스로 이름을 해석하는 클라이언트(tun2proxy)는 A 레코드로 받은 IPv4
            // 리터럴을 보낸다. 폰의 셀룰러에도 IPv6 주소가 없어(실측) NAT64 경로가
            // 생기지 않는다. 그 전제가 깨지면 이 검사를 다시 판단해야 한다.
